...
import requests
from support.serializers import TypeRequestSerializer, DonationCountrySerializer, EmailCitySerializer, EmailCountrySerializer, EmailSerializer, DonationEmailSerializer, HowCanWeHelpYouSerializer,  WhoIAmViewSerializer
from support.models import EmailSupport, IAm, HowCanWeHelpYou, Donation, query_support_email_by_args, query_donation_email_by_args
from django.utils.text import slugify

from support.email_pdf import genrate_email_pdf
from corcym.settings import EMAIL_HOST_USER
# sendgrid
import sendgrid
...
from corcym.settings import PATH_P, DEFAULT_FROM_EMAIL, SENDGRID_API_KEY, COMMUNICATION, COUSTOMER_QUALITY, DONATION_EMAIL
sg = sendgrid.SendGridAPIClient(SENDGRID_API_KEY)
from support.donation_pdf import genrate_donation_pdf
import PyPDF2
import boto3
from botocore.exceptions import NoCredentialsError
from corcym.settings import AWS_STORAGE_BUCKET_NAME, AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY
	...
	def get(self, request, pk=None):
		try:
			if pk is not None:
				...
				return self.send_response(success=True,status_code=status.HTTP_200_OK, payload=serializer.data,code= '',description='Support Email',log_description='')
			data = EmailSupport.objects.all()
			serializer = EmailSerializer(data, many=True)
			print(serializer.data)
			return self.send_response(success=True,status_code=status.HTTP_200_OK, payload=serializer.data,code= '',description='Support Email',log_description='')
		except ObjectDoesNotExist:
			return self.send_response(code='422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description="Support Email doesn't exsist.")
		except EmailSupport.DoesNotExist:
			return self.send_response(code='', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description="Support Email doesn't exsist.")
		except FieldError:
	...
	def post(self, request):
		try:
			...
			if serializer.is_valid():
				...
				a = self.upload_to_aws(local_file =path,bucket = 'corcym', s3_file = f"uploads/support_email_{pdf_data['first_name']}.pdf")
				file_link = f"https://corcym.s3.amazonaws.com/uploads/support_email_{pdf_data['first_name']}.pdf"
				data_dict = {'link':file_link}
				if data['how_can_we_help'] == 'Product feedback' or data['how_can_we_help'] == ' Report a problem with our devices' or data['how_can_we_help'] == 'Patient card and registration form' or data['how_can_we_help'] == 'Collaborate with us':
					res = self.send_mail( template_id, DEFAULT_FROM_EMAIL,  COUSTOMER_QUALITY, data_dict)
					self.send_mail( user_template, DEFAULT_FROM_EMAIL,  [data['email']], data_dict)
					return self.send_response(success=True, code=f'201', status_code=status.HTTP_201_CREATED, description='Email sent to customerquality@corcym.com')
				res = self.send_mail( template_id, DEFAULT_FROM_EMAIL,  COMMUNICATION, data_dict)
				self.send_mail( user_template, DEFAULT_FROM_EMAIL,  [data['email']], data_dict)
				print(res)
				return self.send_response(success=True, code=f'201', status_code=status.HTTP_201_CREATED, description='Email sent to communication@corcym.com')
			return self.send_response(code=f'422',status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description=serializer.errors)
		except EmailSupport.DoesNotExist:
			return self.send_response(code=f'422',status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,description="Booking doesn't exists")
		except FieldError:
			return self.send_response( code=f'500',description="Cannot resolve keyword given in 'order_by' into field")
		except Exception as e:
			print(e)
...
class DonationEmailSendingView(BaseAPIView):
	...
	def concat_pdf(self, path1, path2, data, time):
		...
		pdfOutputFile = open(f"{PATH_P}{time}_{slugify(data['name'])}_donation2pdf.pdf", 'wb')
	...
	def get(self, request, pk=None):
		try:
			if pk is not None:
				...
				return self.send_response(success=True,status_code=status.HTTP_200_OK, payload=serializer.data,code= '',description='Donation Email',log_description='')
			data = Donation.objects.all()
			serializer = DonationEmailSerializer(data, many=True)
			print(serializer.data)
			return self.send_response(success=True,status_code=status.HTTP_200_OK, payload=serializer.data,code= '',description='Donation Email',log_description='')
		except ObjectDoesNotExist:
			return self.send_response(code='422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description="Donation Email doesn't exsist.")
		except Donation.DoesNotExist:
			return self.send_response(code='', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description="Donation Email doesn't exsist.")
		except FieldError:
			return self.send_response(code=f'500', description="Cannot resolve keyword given in 'order_by' into field")
		except Exception as e:
			print(e)
	...
	def post(self, request):
		try:
			...
			if serializer.is_valid():
				...
				s3_file_path = f"uploads/{time}_{slugify(data['name'])}_donation2pdf.pdf"
				a = self.upload_to_aws(local_file =f"{PATH_P}{time}_{slugify(data['name'])}_donation2pdf.pdf",bucket = 'corcym', s3_file = s3_file_path)
				file_link = f"https://corcym.s3.amazonaws.com/{s3_file_path}"
				data_dict = {'link':file_link}
				res = self.send_mail( template_id, DEFAULT_FROM_EMAIL,  DONATION_EMAIL, data_dict)
				# print(res)
				user_template = 'd-b2fba4789d984e89832e2e47b8232364'
				data = serializer.data
				res = self.send_mail( user_template, DEFAULT_FROM_EMAIL,[data['email']] , data_dict)
				print(res)
				return self.send_response(success=True, code=f'201', status_code=status.HTTP_201_CREATED, description='Donation email is created')
			return self.send_response(code=f'422',status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description=serializer.errors)
		except Donation.DoesNotExist:
	...
	def get(self, request, pk=None):
		try:
			if pk is not None:
				...
				return self.send_response(success=True,status_code=status.HTTP_200_OK, payload=serializer.data,code= '',description='Who I am',log_description='')
			iam = IAm.objects.all()
			serializer = WhoIAmViewSerializer(iam, many=True)
			print(serializer.data)
			return self.send_response(success=True,status_code=status.HTTP_200_OK, payload=serializer.data,code= '',description='Who I am',log_description='')
		except ObjectDoesNotExist:
			return self.send_response(code='422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description="No Who I am data exsist.")
		except IAm.DoesNotExist:
			return self.send_response(code='', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description="Who I am doesn't exists")
		except FieldError:
			return self.send_response(code=f'500', description="Cannot resolve keyword given in 'order_by' into field")
		except Exception as e:
			print(e)
	...
	def get(self, request, pk=None):
		try:
			if pk is not None:
				...
				return self.send_response(success=True,status_code=status.HTTP_200_OK, payload=serializer.data,code= '',description='How can we help  you',log_description='')
			data = HowCanWeHelpYou.objects.all()
			serializer = HowCanWeHelpYouSerializer(data, many=True)
			print(serializer.data)
			return self.send_response(success=True,status_code=status.HTTP_200_OK, payload=serializer.data,code= '',description='How can we help  you',log_description='')
		except ObjectDoesNotExist:
			return self.send_response(code='422', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description="No How can we help  you data exsist.")
		except HowCanWeHelpYou.DoesNotExist:
			return self.send_response(code='', status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, description="How can we help  you doesn't exists")
		except FieldError:
			return self.send_response(code=f'500', description="Cannot resolve keyword given in 'order_by' into field")
		except Exception as e:
			print(e)
...
class EmailViewSet(viewsets.ModelViewSet, BaseAPIView):
	...
	def list(self, request, **kwargs):
		try:
			...
			print("restlt:  \n", result)
			return self.send_response(success=True,status_code=status.HTTP_200_OK, payload=result,code= '200',description='Support Email Details',log_description='')	
		except EmailSupport.DoesNotExist:
			return self.send_response(code=f'422',status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,description="Support Email doesn't exists")
		except FieldError:
			return self.send_response( code=f'500',description="Cannot resolve keyword given in 'order_by' into field")
		except Exception as e:
			print(e)
...
class DonationViewSet(viewsets.ModelViewSet, BaseAPIView):
	...
	def list(self, request, **kwargs):
		try:
			...
			donation = query_donation_email_by_args(query_object, **request.query_params)
			...
			print("restlt:  \n", result)
			return self.send_response(success=True,status_code=status.HTTP_200_OK, payload=result,code= '200',description='Donation email Details',log_description='')	
		except Donation.DoesNotExist:
			return self.send_response(code=f'422',status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,description="Donation email doesn't exists")
		except FieldError:
			return self.send_response( code=f'500',description="Cannot resolve keyword given in 'order_by' into field")
		except Exception as e:
			print(e)
...
class EmailCountryView(BaseAPIView):
	...
	def get(self, request, pk=None):
		try:
			...
			country = EmailSupport.objects.values('country').annotate(dcount=Count('country')).order_by()
			print(country)
			...
			return self.send_response(code=f'422',status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,description="EmailSupport doesn't exists")
		except FieldError:
			return self.send_response( code=f'500',description="Cannot resolve keyword given in 'order_by' into field")
		except Exception as e:
			print("Exception: ", e)
...
class EmailCityView(BaseAPIView):
	...
	def get(self, request, pk=None):
		try:
			# jobs = JobPosting.objects.filter(status = True, deleted = False)
			city = EmailSupport.objects.values('city').annotate(dcount=Count('city')).order_by()
			print(city)
...
class SupportGroupByView(BaseAPIView):
	...
	def get(self, request, pk=None):
		try:
			# jobs = JobPosting.objects.filter(status = True, deleted = False)
			country = EmailSupport.objects.values('country').annotate(dcount=Count('country')).order_by().all()
			how_can_we_help = EmailSupport.objects.values('how_can_we_help').annotate(dcount=Count('how_can_we_help')).order_by().all()
			serializer = EmailCountrySerializer(country, many=True)
			...
			return self.send_response(code=f'422',status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,description="Email Support doesn't exists")
		except FieldError:
			return self.send_response( code=f'500',description="Cannot resolve keyword given in 'order_by' into field")
		except Exception as e:
			print("Exception: ", e)
...
class DonationGroupByView(BaseAPIView):
	...
	def get(self, request, pk=None):
		try:
			# jobs = JobPosting.objects.filter(status = True, deleted = False)
			country = Donation.objects.values('country').annotate(dcount=Count('country')).order_by().all()
			type_of_request = Donation.objects.values('type_of_request').annotate(dcount=Count('type_of_request')).order_by().all()
			serializer = DonationCountrySerializer(country, many=True)
			typeserializer = TypeRequestSerializer(type_of_request, many = True)
			data = {
			'country':serializer.data,
			'type_of_request':typeserializer.data
			}
			return self.send_response(success=True,status_code=status.HTTP_200_OK, payload=data,code= '',description='Details of serializer',log_description='')
		except Donation.DoesNotExist:
			return self.send_response(code=f'422',status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,description="Donation doesn't exists")
