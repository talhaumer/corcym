...
from career.serializers import (
    ...
    EmailDataSerializer,
    JobCityListingSerializer,
    JobListingSerializer,
    JobLocationSerializer,
    JobPostingSerializer,
    JobTitleSerializer,
)
from corcym.settings import ADMIN_CAREER, DEFAULT_FROM_EMAIL, SENDGRID_API_KEY
    ...
    def post(self, request):
        try:
            ...
            if serializer.is_valid():
                ...
                templates = {
                    "admin": "d-a0e4337ce6864cdd8c11a6be5b0a2f68",
                    "candidate": "d-b763d599c2e6411897d779255f518197",
                }
                data_dict = {
                    "jobLink": "https://corcym.com/detail-job/{}".format(
                        serializer.data["job_id"]
                    ),
                ...
                a = self.upload_to_aws(
                    local_file=f"{PATH_P}{data_dict['applicant_name']}sresume.pdf",
                    bucket="corcym",
                    s3_file=f"uploads/{data_dict['applicant_name']}sresume.pdf",
                )
                data_dict[
                    "jobLink"
                ] = f"https://corcym.s3.eu-central-1.amazonaws.com/uploads/{data_dict['applicant_name']}sresume.pdf"
                mail_res = self.send_mail(
                    templates["admin"], DEFAULT_FROM_EMAIL, ADMIN_CAREER, data_dict
                )
                mail_res2 = self.send_mail(
                    templates["candidate"],
                    DEFAULT_FROM_EMAIL,
    ...
    def put(self, request, pk=None):
        try:
            ...
            serializer = JobPostingSerializer(
                instance=saved_jobs, data=data, partial=True
            )
            if serializer.is_valid():
                JobPostingSaved = serializer.save()
                return self.send_response(
            ...
            return self.send_response(
                code="422",
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                description="No job matches the given query.",
            )
        except JobPosting.DoesNotExist:
            return self.send_response(
                code=f"422",
    ...
    def get(self, request, pk=None):
        try:
            if pk is not None:
                ...
                if job.status == False:
                    return self.send_response(
                        description="Job disabled successfuly",
                        success=True,
                        status_code=status.HTTP_200_OK,
                        code="200",
                    )
                return self.send_response(
                    description="Job enabled successfuly",
                    success=True,
                    status_code=status.HTTP_200_OK,
                    code="200",
                )
    ...
    def get(self, request, pk=None):
        try:
            ...
            return self.send_response(
                code=f"422",
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                description="candidate doesn't exists",
            )
        except FieldError:
            return self.send_response(
                code=f"500",
        ...
        try:
            ...
            if not jobs:
                return self.send_response(
                    ...
                    payload={},
                    code="",
                    description="query_jobs_by_args Functions returning None",
                    log_description="",
                )
    ...
    def post(self, request):
        try:
            ...
            if serializer.is_valid():
                ...
                mail_res = self.send_mail(
                    "d-d4c7188e448e42129b055c83521dd6fe",
                    DEFAULT_FROM_EMAIL,
                    ["marcello.oppezzo@livanova.com"],
                    data_dict,
                )
                mail_res = self.send_mail(
                    "d-d4c7188e448e42129b055c83521dd6fe",
                    DEFAULT_FROM_EMAIL,
                    ["gemma.vergara@livanova.com"],
                    data_dict,
                )
                mail_res = self.send_mail(
                    "d-d4c7188e448e42129b055c83521dd6fe",
                ...
                print(mail_res)
                mail_res2 = self.send_mail(
                    "d-b763d599c2e6411897d779255f518197",
                    DEFAULT_FROM_EMAIL,
                    request.data["email"],
                    data_dict={},
                )
            ...
            return self.send_response(
                code=f"422",
                status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                description="Email Data  doesn't Post",
            )
        except FieldError:
            return self.send_response(
                code=f"500",
    ...
    def get(self, request, pk=None):
        try:
            jobs = (
                ...
                .annotate(dcount=Count("location"))
                .order_by()
                .all()
            )
            job_title = (
                JobPosting.objects.values("job_title")
                .annotate(dcount=Count("job_title"))
                .order_by()
                .all()
            )
            print(job_title)
            ...
            data = {"location": serializer.data, "title": job_title_ser.data}
            return self.send_response(
                success=True,
                status_code=status.HTTP_200_OK,
                payload=data,
    ...
    def get(self, request, pk=None):
        try:
            ...
            if pk is not None:
                ...
                return self.send_response(
                    description=" Candidate Deleted successfuly",
                    success=True,
                    status_code=status.HTTP_200_OK,
                    code="200",
                )
...
class CandidateGroupByView(BaseAPIView):
    ...
    def get(self, request, pk=None):
        try:
            country = (
                CandidateDetail.objects.values("job")
                .annotate(dcount=Count("job"))
                .order_by()
                .all()
            )
...
class JobCityListingView(BaseAPIView):
    ...
    def get(self, request, pk=None):
        try:
            jobs = (
                JobPosting.objects.filter(status=True, deleted=False)
                .values("location", "job_title")
                .distinct()
            )
